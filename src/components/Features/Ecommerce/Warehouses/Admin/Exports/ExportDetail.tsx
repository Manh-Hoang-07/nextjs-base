...
export default function ExportDetail({ id }: ExportDetailProps) {
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const res = await apiClient.get(adminEndpoints.warehouseExports.show(id));
            setData(res.data.data);
        } catch (error: any) {
            showError("Không thể tải thông tin phiếu xuất");
            router.push("/admin/warehouse-exports");
        } finally {
            setLoading(false);
        }
    }, [id, showError, router]);
    ...
    const handleApprove = async () => {
        try {
            ...
            showError(error?.response?.data?.message || "Lỗi khi duyệt phiếu. Kiểm tra tồn kho!");
        }
    };
        ...
        <div className="export-detail">
            ...
            <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className="block text-sm text-gray-500 mb-1">Kho Xuất</label>
                        <div className="font-medium text-gray-900 border p-3 rounded bg-gray-50">
                            {data.from_warehouse?.name || `Warehouse ID: ${data.from_warehouse_id}`}
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        <label className="block text-sm text-gray-500 mb-1">Số lượng xuất</label>
            ...
            <ConfirmModal
                ...
                message={`Bạn có chắc chắn muốn duyệt phiếu xuất này? Tồn kho sẽ được cập nhật.`}
