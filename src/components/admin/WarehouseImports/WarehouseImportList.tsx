...
export default function WarehouseImportList() {
    ...
    const {
        ...
        endpoints: {
            list: adminEndpoints.warehouseImports.list,
            create: adminEndpoints.warehouseImports.create,
        },
        messages: {
    ...
    const handleApprove = useCallback(async (id: number) => {
        try {
            await apiClient.post(adminEndpoints.warehouseImports.approve(id)); // Doc says POST for approve
    ...
    const handleCancel = useCallback(async (id: number) => {
        try {
            await apiClient.post(adminEndpoints.warehouseImports.cancel(id)); // Doc says POST for cancel
    ...
    return (
        <div className="warehouse-imports">
            ...
            <ImportFilter initialFilters={filters} onUpdateFilters={updateFilters} />
                                ...
                                {items.map((item, index) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {item.to_warehouse?.name || `ID: ${item.to_warehouse_id}`}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <Actions
                                                item={item}
                                                showView={true}
                                                onView={() => router.push(`/admin/warehouse-imports/${item.id}`)}
                                                showEdit={false}
                                                showDelete={false}
                                                additionalActions={[
                                                    ...(item.status === 'pending' ? [{
            ...
            {selectedItem && (
                <>
                    <ConfirmModal
                        ...
                        message={`Bạn có chắc chắn muốn duyệt phiếu nhập "${selectedItem.code}"? Kho hành động này không thể hoàn tác.`}
